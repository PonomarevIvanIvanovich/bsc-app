    ...
    func goBackButton(model: NotesModel?, index: Int?) {
        ...
        if let index = index {
            if filter.arrayModel.indices.contains(index) {
                let oldModel = filter.arrayModel[index]
                if oldModel.userShareIcon != newModel.userShareIcon {
                    newModel.userShareIcon = oldModel.userShareIcon
                }
                filter.arrayModel[index] = newModel
            } else {
                filter.arrayModel.insert(newModel, at: 0)
            }
            noteList.reloadData()
        } else {
            filter.arrayModel.insert(newModel, at: 0)
